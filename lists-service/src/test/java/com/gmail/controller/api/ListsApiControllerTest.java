...
public class ListsApiControllerTest {
    ...
    public void getNotificationList() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(PathConstants.API_V1_LISTS + PathConstants.LIST_ID, 4)
    ...
    public void getTweetList() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(PathConstants.API_V1_LISTS + PathConstants.TWEET_LIST_ID, 4)
                        .header(PathConstants.AUTH_USER_ID_HEADER, TestConstants.USER_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(4))
                .andExpect(jsonPath("$.listName").value(TestConstants.LIST_NAME))
